...
const FormSteps = (props: Props) => {
    ...
    return (
        <>
            ...
            <HorizontalNavFooter
                mainAction={() =>
                    submit().then(() => history.push(`${adminRoute}/collections/email-template`))
                }
                canDoMainAction
                mainText="Save and Quit"
                quitText="Quit Without Saving"
                showAutosave={false}
                quit={() => history.push(`${adminRoute}/collections/email-template`)}
            />
        </>
    );
};

const RenderEmailTemplateFlowFields: React.FC<Props> = props => {
...
export default RenderEmailTemplateFlowFields;
