...
import useThrottledEffect from 'payload/dist/admin/hooks/useThrottledEffect';
import { fieldReducer } from 'payload/dist/admin/components/forms/Form/fieldReducer';
import initContextState from 'payload/dist/admin/components/forms/Form/initContextState';
import reduceFieldsToValues from 'payload/dist/admin/components/forms/Form/reduceFieldsToValues';
import getSiblingDataFunc from 'payload/dist/admin/components/forms/Form/getSiblingData';
import getDataByPathFunc from 'payload/dist/admin/components/forms/Form/getDataByPath';
import wait from 'payload/dist/utilities/wait';
import { Field } from 'payload/types';
import buildInitialState from 'payload/dist/admin/components/forms/Form/buildInitialState';
import errorMessages from 'payload/dist/admin/components/forms/Form/errorMessages';
import {
    Context,
    Fields,
    ...
    Row,
    SubmitOptions,
} from 'payload/dist/admin/components/forms/Form/types';
import {
    SubmittedContext,
    ProcessingContext,
    ModifiedContext,
    ...
    FormFieldsContext,
    FormWatchContext,
} from 'payload/dist/admin/components/forms/Form/context';
...
import { WatchFormErrors } from 'payload/dist/admin/components/forms/Form/WatchFormErrors';
import { splitPathByArrayFields } from 'payload/dist/utilities/splitPathByArrayFields';
...
const Form: React.FC<Props> = props => {
            ...
            for (let i = 0; i < pathSegments.length; i += 1) {
                ...
                if (Array.isArray(formField?.rows)) {
                    ...
                    if (field.valid === false && childFieldPath) {
                        newFieldRows[fieldPath][rowIndex].childErrorPaths.add(
        ...
        ({
            ...
            for (let i = 0; i < pathSegments.length; i += 1) {
                ...
                if (isNumber(pathSegment)) {
                    ...
                    const arrayFieldPath = pathPrefix
                        ? `${pathPrefix}.${parentFieldPath}`
                        : parentFieldPath;
                    ...
                    if (rowField.blockType) {
                        const blockConfig = configMap.get(
                            `${parentFieldPath}.${rowField.blockType}`
                        );
                        if (blockConfig) {
                        ...
                        throw new Error(
                            `Block config not found for ${rowField.blockType} at path ${path}`
                        );
                    } else {
                        return traverseRowConfigs({
                            pathPrefix: `${arrayFieldPath}.${rowIndex}`,
    ...
    const getRowConfigByPath = React.useCallback(
        ({ path, blockType }: { path: string; blockType?: string }) => {
